...
NodoArbolAVL<T> *ArbolBinarioAVL<T>::crearEspejo() {
    return crearespejo(root);
}
template <class T>
NodoArbolAVL<T>*ArbolBinarioAVL<T>:: crearespejo(NodoArbolAVL<T>* root) {
    if (root == nullptr) {
        return nullptr;
    }

    // Intercambiar los subárboles izquierdo y derecho
    NodoArbolAVL<T>* temp = root->getLeft();
    root->setLeft(crearespejo(root->getRight()));
    root->setRight(temp);
...
template<class T>
int ArbolBinarioAVL<T>::contarPorNivel(int nivelDeseado) {
    return contarporNivel(root, nivelDeseado);
}
template<class T>
int ArbolBinarioAVL<T>::contarporNivel(NodoArbolAVL<T> *raiz, int nivelDeseado) {
    if (raiz == nullptr) {
        return 0; // Si el nodo es nulo, no hay nodos en este nivel
    }

    if (nivelDeseado == 0) {
        return 1; // Si el nivel deseado es 0 y el nodo no es nulo, hay un nodo en este nivel
    }

    // Recursivamente contar nodos en el nivel izquierdo y derecho
    int nodosIzquierda = contarporNivel(raiz->getLeft(), nivelDeseado - 1);
    int nodosDerecha = contarporNivel(raiz->getRight(), nivelDeseado - 1);

    // Sumar los nodos en el nivel izquierdo y derecho
...
int ArbolBinarioAVL<T>::contarNodos(NodoArbolAVL<T>* r) {
    ...
    // Contar el nodo actual y sumar los nodos en los subárboles izquierdo y derecho
